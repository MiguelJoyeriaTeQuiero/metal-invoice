...
import type { PurchaseInvoiceListItem, PurchaseStatus } from '@/types/purchase-invoice';
...
export function PurchaseInvoiceList() {
  ...
  const [filters, setFilters] = useState<Filters>({ search: '', status: 'ALL', page: 1, pageSize: 20 });
  ...
  const fetchInvoices = useCallback(async () => {
    ...
    try {
      ...
      setFetchError(err instanceof Error ? err.message : 'Error al cargar las facturas');
      setInvoices([]);
      setTotal(0);
    } finally {
      setLoading(false);
      ...
      <div className="px-4 sm:px-6 py-4 border-b border-slate-100 flex flex-wrap gap-3 items-center">
        <div className="relative flex-1 sm:flex-none">
          ...
          <input
            ...
            className="pl-9 pr-4 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1d4f91] w-full sm:w-64"
          />
        </div>
        <select
          value={filters.status}
          onChange={(e) => updateFilter('status', e.target.value)}
          className="px-3 py-2 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1d4f91] flex-1 sm:flex-none"
        >
          {STATUS_OPTIONS.map((o) => (
            <option key={o.value} value={o.value}>{o.label}</option>
          ))}
        </select>
        <span className="ml-auto text-xs text-slate-400 whitespace-nowrap">{total} resultados</span>
      </div>
            ...
            {loading ? (
              ...
              <tr>
                <td colSpan={7} className="px-6 py-16 text-center">
                  ...
                  <p className="text-slate-500 text-sm">No se encontraron facturas de compra</p>
                </td>
              </tr>
            ) : (
              invoices.map((inv) => (
                <tr key={inv.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-sm text-slate-700">{inv.supplier.name}</td>
                  ...
                  <td className="px-6 py-4 text-sm text-slate-500">
                    {inv.paymentMethod === 'TRANSFER' ? 'Transferencia' : 'Tarjeta'}
                  </td>
                  ...
                  <td className="px-6 py-4">
                    <Link href={`/compras/${inv.id}`} className="inline-flex items-center gap-1 text-sm text-[#1d4f91] hover:underline">
